...
async function loadData(){
  ...
  try{
    ...
    data=await response.json();statusMessage.textContent="";setGate(false);render();
  }catch(error){statusMessage.textContent=data===null?"Could not load vocabulary":"Refresh failed · try again";if(data===null){gateError.textContent="Could not load vocabulary";setGate(true)}}
}
